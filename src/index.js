...
async function main() {
    ...
    const analyzer = new analyzer_1.TitoMetralletaAnalyzer(process.env.ALPHA_VANTAGE_KEY || '', process.env.FINNHUB_KEY || '');
    ...
    const report1 = await analyzer.analyzeOpportunity('AAPL', 'Momentum Intraday', plan1);
    if (report1) {
    ...
    const report2 = await analyzer.analyzeOpportunity('TSLA', 'Breakout Strategy', plan2);
    if (report2) {
    ...
    if (report1) {
        ...
        const tradeResult = analyzer
            .getReportEngine()
            .recordTradeResult(report1, 'ganancia', ['Tendencia favorable', 'Volumen confirmó'], [], ['La confirmación en volumen es crucial']);
