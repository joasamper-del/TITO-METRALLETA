...
exports.runDemoAnalysis = runDemoAnalysis;
const analyzer_1 = require("../core/analyzer");
/**
 * Crea datos de mercado simulados para demostración
 */
function createDemoMarketData(symbol, options) {
    return {
        symbol,
        price: 150.5,
        volume: 2500000,
 ...
 */
function createDemoMarketContext() {
    return {
...
async function runDemoAnalysis() {
    ...
    console.log('╚═══════════════════════════════════════════════════════════╝\n');
    const analyzer = new analyzer_1.TitoMetralletaAnalyzer('', '');
    // ============================================================
    ...
    const favorableAnalysis = rulesEngine.analyzeData(favorableData, context, 'Momentum Intraday');
    ...
    console.log(`🎯 Decisión: ${favorableAnalysis.decision.toUpperCase()}`);
    ...
    const favorableReport = reportEngine.generateReport(favorableAnalysis, 'Momentum Intraday', favorablePlan);
    ...
    const unfavorableAnalysis = rulesEngine.analyzeData(unfavorableData, context, 'Mean Reversion');
    ...
    console.log(`🎯 Decisión: ${unfavorableAnalysis.decision.toUpperCase()}`);
    ...
    const customAnalysis = rulesEngine.analyzeData(favorableData, context, 'Momentum Intraday');
    console.log(`Puntuación anterior: ${favorableAnalysis.percentageScore.toFixed(0)}%`);
    ...
    const customAnalysis2 = rulesEngine.analyzeData(favorableData, context, 'Momentum Intraday');
    ...
    console.log('─────────────────────────────────────────────────');
    const tradeResult = reportEngine.recordTradeResult(favorableReport, 'ganancia', ['Confirmación de volumen', 'Ruptura limpia de resistencia'], [], [
    ...
    console.log('🔧 Todos los componentes son modulares y fáciles de extender.\n');
}
