...
class ReportEngine {
     ...
     */
    generateReport(analysis, strategy, plan) {
        return {
            id: this.generateId(),
     ...
     */
    generateManualReviewReport(symbol, strategy, missingData) {
        return {
            id: this.generateId(),
            symbol,
            strategy,
     ...
     */
    recordTradeResult(report, result, successReasons = [], failureReasons = [], lessons = []) {
        const tradeResult = {
            reportId: report.id,
    ...
    generatePerformanceStats(results) {
        ...
        const avgPointsWin = wins.length > 0
            ? wins.reduce((sum, r) => sum + (r.points || 0), 0) / wins.length
            : 0;
        const avgPointsLoss = losses.length > 0
            ? losses.reduce((sum, r) => sum + (r.points || 0), 0) / losses.length
            : 0;
        const bestTrade = results.reduce((best, current) => (current.points || 0) > (best.points || 0) ? current : best);
        const worstTrade = results.reduce((worst, current) => (current.points || 0) < (worst.points || 0) ? current : worst);
        return {
