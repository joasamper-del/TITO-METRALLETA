...
class DataEngine {
     ...
     */
    async fetchAlphaVantageData(symbol, marketData) {
        try {
            ...
            const response = await axios_1.default.get(url, { params, timeout: 5000 });
     ...
     */
    async fetchRSI(symbol, marketData) {
        try {
            const url = `https://www.alphavantage.co/query`;
            const params = {
                function: 'RSI',
            ...
            if (data) {
                const latestRSI = Object.values(data)[0];
     ...
     */
    async fetchTrend(symbol, marketData) {
        try {
            const url = `https://www.alphavantage.co/query`;
            const params = {
                function: 'SMA',
            ...
            if (data && marketData.price) {
                const sma20 = parseFloat(Object.values(data)[0]);
                marketData.trend =
     ...
     */
    async fetchFinnhubData(symbol, marketData) {
        try {
            ...
            if (data.c)
                marketData.price = data.c;
            if (data.v)
                marketData.volume = data.v;
        }
        catch (error) {
