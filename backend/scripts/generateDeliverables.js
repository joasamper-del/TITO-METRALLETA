...
function generatePerformanceReport() {
    ...
    for (const d of decisions) {
        ...
        if (d.confidence <= 20)
            stats.confidenceDistribution["0-20"]++;
        else if (d.confidence <= 40)
            stats.confidenceDistribution["21-40"]++;
        else if (d.confidence <= 60)
            stats.confidenceDistribution["41-60"]++;
        else if (d.confidence <= 80)
            stats.confidenceDistribution["61-80"]++;
        else
            stats.confidenceDistribution["81-100"]++;
    }
...
function generateOperationsLog() {
    ...
    const logEntries = sample
        .map((d, i) => `${d.index.toString().padStart(4)} | ${d.timestamp} | ${d.symbol.padEnd(5)} | ${d.status.padEnd(20)} | ${d.confidence.toString().padStart(3)}% | ${d.razones[0].substring(0, 60)}`)
        .join("\n");
...
` ``;
    ...
    claras;
    en;
    types_1.DecisionDetails ? (Clarity) `` `
