...
interface Observation {
  ...
  symbol: string;
  currentPrice: number;
  priceSource: 'alpaca-positions' | 'missing';
  priceFreshness: 'REAL' | 'UNKNOWN';
  entryPrice?: number;
  pnl?: number;
  pnlPercent?: number;
  quantity?: number;
  ...
  rsi: 'MISSING';
  atr: 'MISSING';
  proposedSignal: 'HOLD' | 'WAIT' | 'UNKNOWN';
  reason: string;
}
...
class ObservationEngine {
  ...
  private logsDir = path.join(__dirname, '../audit/s60-observations');
  ...
  private async observationLoop(): Promise<void> {
  ...
  private async collectObservation(): Promise<void> {
    this.cycleCount++;
    const now = new Date();
    ...
    try {
      ...
      const btcPos = positions.data.find((p: any) => p.symbol === 'BTCUSD');
      if (!btcPos) {
        const btcObs: Observation = {
          cycleNumber: this.cycleCount,
          timestamp: now.toISOString(),
          unixTime: now.getTime(),
          symbol: 'BTCUSD',
  ...
  private displayProgress(observations: Observation[]): void {
  ...
  private saveStatus(status: string, time: Date): void {
